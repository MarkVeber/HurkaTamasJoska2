...
module.exports = {
  name: "play",
  aliases: ["p"],
  description: "Zene lejátszása youtube/soundcloud -rol.",
  cooldown: 1.5,
  edesc: `Írd be, hogy zenét játsz le.\nUsage: ${PREFIX}play <TITLE | URL>`,
...
async execute(message, args, client) {
    ...
    if (!channel) return attentionembed(message, "Először menj be egy hangcsatornába.");
    ...
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Kell jog, hogy bemenjek a csatornába!");
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Keresem... 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Keresem... 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
    ...
    if (urlValid) {
      try {
        ...
        if (error.statusCode === 403) return attentionembed(message, "ERROR");
        console.error(error);
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .addField("Pozíció a listán:", `**\`${serverQueue.songs.length - 1}\`**`, true)
    ...
    try {
      ...
      return attentionembed(message, `Nem tudok bemenni a csatornába: ${error}`);
    }
  }
};
