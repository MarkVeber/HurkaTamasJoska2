...
module.exports = {
  name: "radio",
  description: "nyomjad a rádiót heeeeee",
  cooldown: 3,
  edesc: `Nymojad a commandot, hogy rádiót halgass!\nUsage: ${PREFIX}radio <1-34>`,
  async execute(message, args, client) {
    //define the No args Embed, lmao
    let resultsEmbed = new Discord.MessageEmbed()
      .setTitle(`**Radio állomások (letöltött script + 1-2 hozzáadott mint pl rádió-1)**`)//
      .addFields(
        {
...
**34: ** [\`${Radiostations[34 - 1].split(" ")[0]}\`](${Radiostations[34 - 1].split(" ")[1]})`, inline: true
    ...
    if (!channel) return attentionembed(message, "Lépj be hangcsatornába először!");
    //react with emoji
    message.react(approveemoji);
    //If not in the same channel return error
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return attentionembed(message, `Nem vagyunk ugyanazon csatornában!`);
    //check permissions
    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Kell jog, hogy bejöjek!");
    if (!permissions.has("SPEAK"))
      return attentionembed(message, "Kell jog, hogy beszéljek!");
    ...
    if (isNaN(args[0])) {
      ...
      return message.reply(
        new MessageEmbed()
          .setColor("#F0EAD6")
          .setTitle(`Nem valid, \`1\` és \`${Radiostations.length}\` között!`)
      );
    }
    ...
    message.channel.send(new MessageEmbed().setColor("#F0EAD6")
      .setDescription(`**:radio: Keresem 🔍 \`${Radiostations[i - 1].split(" ")[0]}\`**`));
    //mute yourself
    ...
    */
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .setURL(song.url)
        .setDescription(`\`\`\`Hozzáadtam a listához.\`\`\``)
        .addField("Idő a lejátszásig:", `\`${estimatedtime}\``, true)
        .addField("Pozíció a listában", `**\`${serverQueue.songs.length - 1}\`**`, true)
        .setFooter(`Kérte: ${message.author.username}#${message.author.discriminator}`, message.member.user.displayAvatarURL({ dynamic: true }))
