...
module.exports = {
  name: "filter",
  description: "filter változtatása.",
  aliases: ["f"],
  cooldown: 3,
  edesc: `Írd be, hogy megváltoztasd a filtert, a "nullázáshoz" a clear filtert rakd be. \nUsage: ${PREFIX}filter <Filtertype>`,
  ...
  async execute(message, args, client) {
    ...
    if (message.channel.activeCollector)
      return attentionembed(message, "Már keresem te vadállat!");
    //if the user is not in a voice channel return error
    if (!message.member.voice.channel)
      return attentionembed(message, "Menj hangcsatornába először.")
    ...
    switch (args[0]) {
      ...
      default:
        ...
        .setColor("#F0EAD6")
        .setTitle("Nem jó filter, ezek vannak:")
        .setDescription(`
        \`bassboost\`
        \`8D\`
        \`vaporwave\`
        ...
        .setFooter(`Példa: ${PREFIX}filter bassboost`)
        .setFooter(`filter list by: ZerioDev`)
        )
        break;
    }
    //set choice to zero
