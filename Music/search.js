...
module.exports = {
  name: "search",
  description: "Keress, és válassz zenét a lejátszáshoz.",
  aliases: ["s"],
  cooldown: 3,
  edesc: `Írd be, hogy megnézd az első öt találatot a zenére\nUsage: ${PREFIX}search <Cím/URL>`,
...
async execute(message,args,client) {
    ...
    if (!args.length)
      return attentionembed(message,`Használat: ${message.client.prefix}${module.exports.name} <Video Name>`)
    //if there is already a search return error
    if (message.channel.activeCollector)
      return attentionembed(message,"Nincs találat!");
    //if the user is not in a voice channel return error
    if (!message.member.voice.channel)
      return attentionembed(message,"Menj csatornába először.")
       //If not in the same channel return error
    if (serverQueue && channel !== message.guild.me.voice.channel)
    return attentionembed(message, `Nem vagy egy csatornában velem.`);
    //define search
    ...
    let resultsEmbed = new MessageEmbed()
      ...
      .setColor("#F0EAD6")
      .setAuthor("Eredmények Józsi!!")
      .setFooter("Válaszolj a számmal")
    ...
    try {
      ...
      if(response === "error"){
        //send error message
        attentionembed(message,"Használj másik emojit!");
