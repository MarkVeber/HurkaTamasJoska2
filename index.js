...
client.on(`ready`, () => {	
   ...
   ///////////////////////////////
    ...
    figlet.text(`${client.user.username} ready!`, function (err, data) {
      if (err) {
          console.log('Baj van CSHORRO');
          console.dir(err);
      }
      console.log(`im alááájv`);
    })
...
client.on(`message`, async (message) => {
  ...
  if(message.content.startsWith(`${prefix}embed`)){
    ...
    .setFooter("HurkaTamasJoska2", client.user.displayAvatarURL())
    //delete the Command
...
//command Handler DO NOT TOUCH
 ...
 if (timestamps.has(message.author.id)) {
   ...
   if (now < expirationTime) {
     ...
     return message.reply(
      ...
      .setTitle(`:x: Várján \`${timeLeft.toFixed(1)} másodpercet\` mielőtt használnád a commandot: \`${prefix}${command.name}\`!`)    
     );
   }
 }
 ...
 try {
   ...
   .setTitle(`:x: ERROR VAN CSHORRO.`)).catch(console.error);
 }
